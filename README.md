...
- Добавить dictionary-режим для валют:
  - хранить список допустимых `code -> name -> sign`;
  - при создании валюты проверять, что код есть в словаре;
  - опционально подставлять `name/sign` из словаря автоматически.
- Добавить выпадающий список в UI:
  - при добавлении курса и конвертации давать выбор из доступных кодов, а не свободный ввод.
- Ужесточить валидацию входных данных:
  - единый формат ошибок (`code`, `message`, `status`, `details`) уже есть — можно расширить `details` по полям;
  - добавить дополнительные интеграционные тесты на 4xx/5xx сценарии.
- Усилить защиту публичного API:
  - soft rate-limit на уровне reverse proxy;
  - логирование подозрительных запросов и периодическая очистка мусорных данных.
