...
public final class PathUtils {
    ...
    public static String extractCurrencyCode(HttpServletRequest request) {
        ...
        if (code.length() != (ValidationUtils.CURRENCY_PAIR_LENGTH)/2) {
    ...
    public static String[] extractCurrencyPair(HttpServletRequest req) {
        ...
        if (pathInfo.length() != ValidationUtils.CURRENCY_PAIR_LENGTH) {
            log.warn("Currency pair length is not {}: {}", ValidationUtils.CURRENCY_PAIR_LENGTH, pathInfo);
            throw new ValidationException(
                    String.format("Currency pair must be %d characters long", ValidationUtils.CURRENCY_PAIR_LENGTH)
            );
        }

        String baseCode = pathInfo.substring(0, (ValidationUtils.CURRENCY_PAIR_LENGTH)/2);
        String targetCode = pathInfo.substring((ValidationUtils.CURRENCY_PAIR_LENGTH)/2, ValidationUtils.CURRENCY_PAIR_LENGTH);
